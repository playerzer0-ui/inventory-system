...
<main class="main-container">
    <form id="myForm" action="{{ route('amend_payment_data') }}" method="post">
        ...
        <table>
            ...
            <tr class="highlight">
                @if ($state == 'moving')
                    ...
                    <td>Invoice Date</td>
                    <td>:</td>
                    <td colspan="2"><input name="invoice_date" type="date" id="invoice_date" placeholder="fill in" value="{{$invoice['invoice_date']}}" readonly></td>
                @else
                    @if ($state == "in")
                        <td>No SJ</td>
                        <td>:</td>
            ...
            <tr>
                ...
                @else
                    <td>Invoice Date</td>
                    <td>:</td>
                    <td colspan="2"><input name="invoice_date" type="date" id="invoice_date" placeholder="fill in" oninput="generateNoInvoice()" value="{{$invoice['invoice_date']}}" readonly></td>
                    <td colspan="4"></td>
                @endif
            </tr>
            @endif
                ...
                <tr>
                    ...
                    <td>
                        <input type="number" value="{{ $key->price_per_UOM }}" inputmode="numeric" 
                            name="price_per_uom[]" placeholder="Fill in" oninput="calculateNominal(this)" readonly>
                    </td>
                    <td>
        ...
        <table id="accountTable">
            <tr>
                <th>Payment Date:</th>
                <td><input type="date" name="payment_date" id="payment_date" placeholder="fill in" value="{{$payment['payment_date']}}" required></td>
            ...
            <tr>
                ...
                <td>
                    <input type="number" inputmode="numeric" name="payment_amount" id="payment_amount" value="{{$payment['payment_amount']}}" oninput="calculateDebt()" required>
                </td>
                <td>Tax(%): <input type="number" name="tax" id="tax" value="{{$invoice['tax']}}" oninput="calculateTotalNominal()" readonly></td>
